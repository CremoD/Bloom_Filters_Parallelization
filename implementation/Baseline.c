...
int main(int argc, char* argv[]){
	...
	BloomFilter filter = getBloomFilter(filterSize,numberSeeds, parentSeed);
	...
	if(strstr(argv[1], "numeric") && strstr(argv[2], "numeric")){
		...
		*/
		//experiment(filter, input, test, m, &get_size, "numerical");
		...
		D fprintf(stderr, "Dataset is string\n");
		experiment(filter, input, test, dims[0], &strlen, dataset_name);
